...
(function ($) {
	...
	$.extend(Countdown.prototype, {
     ...
     @param  options  (object) the initial settings for the countdown */
		_attachPlugin: function _attachPlugin(target, options) {
			target = $(target);
     ...
     @return  (any) if retrieving a value */
		_optionPlugin: function _optionPlugin(target, options, value) {
			target = $(target);
     ...
     @param  recalc  (boolean) true if until or since are set */
		_adjustSettings: function _adjustSettings(target, inst, recalc) {
			var now;
			var serverOffset = 0;
     ...
     @param  target  (element) the containing division */
		_destroyPlugin: function _destroyPlugin(target) {
			target = $(target);
			if (!target.hasClass(this.markerClassName)) {
				return;
			}
     ...
     @param  target  (element) the containing division */
		_pausePlugin: function _pausePlugin(target) {
     ...
     @param  target  (element) the containing division */
		_lapPlugin: function _lapPlugin(target) {
			this._hold(target, 'lap');
		},

		/* Resume a paused countdown widget.
     @param  target  (element) the containing division */
		_resumePlugin: function _resumePlugin(target) {
     ...
     @return  (number[7]) the current periods for the countdown */
		_getTimesPlugin: function _getTimesPlugin(target) {
			var inst = $.data(target, this.propertyName);
